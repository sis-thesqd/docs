...
├── 1. Config Script (fetch all configs)
├── 2. Task Script (validate & fetch task data)
├── 3. Account Script (fetch designer preferences)
├── 4. Assignees Script (calculate availability)
├── 5. Decision Script (select final designer)
├── 6. Queue Script (move tasks to queue when no room)
└── 7. Assign Script (apply assignment to ClickUp) [NEW 2025-12-18]
```
...
sequenceDiagram
    ...
    Note over Trigger,Assign: ROUTER PHASE

    Trigger->>Router: task_id

    par Parallel Fetch
        Router->>ClickUp API: GET /task/{id}
        Router->>Supabase: get_task_auto_assign_data RPC
        Router->>Supabase: SELECT designer_scheduling
    end

    Router->>Router: Classify task

    alt auto_schedule classification
        Note over Router,Supabase: Time Estimate (Parallel)
        par Set Estimate
            Router->>ClickUp API: PUT /task/{id} (time_estimate)
            Router->>Supabase: UPSERT as_log
            Router->>Supabase: INSERT task_update_log
        end

        Note over Router,Assign: MASTER PHASE

        Router->>Master: task_id

        par Parallel Data Fetch
            Master->>Supabase: SELECT sis_config
            Master->>Supabase: as_get_task_processing_data RPC
            Master->>Supabase: as_get_task_assignment_data RPC
            Master->>ClickUp API: GET /task/{id}
        end

        Master->>Master: Process task + account data

        Master->>Supabase: UPSERT as_log (task data)

        Master->>Supabase: as_get_assignees_data RPC

        Master->>Supabase: UPSERT as_designer_availabilities

        Master->>Master: Decision logic (select designer)

        Master->>Supabase: UPSERT as_log (decision)

        Note over Master,Assign: ASSIGN PHASE

        Master->>Assign: task_id, selected_designer

        Assign->>Supabase: Fetch non_employee_assignees

        Assign->>ClickUp API: PUT /task/{id} (assignee, due_date, status)

        Assign->>ClickUp API: DELETE /task/{id}/tag/queued

        par Logging (Parallel)
            Assign->>Supabase: INSERT task_update_log
            Assign->>Supabase: UPSERT as_log
        end

        Assign->>ClickUp API: POST /task/{id}/comment

    else queued classification
        Router->>Supabase: SELECT queue count
        Router->>ClickUp API: POST /task/{id}/tag/queued
        Router->>Supabase: UPSERT as_log (queued)
        Router->>ClickUp API: POST /task/{id}/comment
    end
```
...
|---------------|---------------------|
| `auto_schedule` | Calls master.py → assign.py for the current task |
| `queued` | Calls queue.py with pre-provided data (avoids duplicate RPCs) |
| `process_queued_tasks` | Loops through queued_task_ids, calls master → assign for each (limited by room) |
| `move_subtasks` | Loops through subtask_ids, calls master → assign for each (limited by room) |
| `move_dependent_tasks` | Loops through dependent_task_ids, calls master → assign for each (limited by room) |

**Error Handling**: Failed tasks are recorded in the `orchestration_result` but don't stop processing of remaining tasks.

**Narrative Enrichment** (Updated 2025-12-19):

The router narrative now includes detailed decision reasoning from the decision script:

**Before** (sparse narrative):
```
"Task 'AA26 Test 8' (86dyy2f5k) with status 'open' was analyzed. Found: task is ready for auto-scheduling with no blocking conditions. Account 306 capacity: 7/8 active tasks, room for 1 more. Decision: process=True because task is ready for auto-scheduling. Orchestration complete: assigned to Marcos Monti (Graphic Designer) with due date 2026-01-13."
```

**After** (includes decision context):
```
"Task 'AA26 Test 8' (86dyy2f5k) with status 'open' was analyzed. Found: task is ready for auto-scheduling with no blocking conditions. Account 306 capacity: 7/8 active tasks, room for 1 more. Decision: process=True because task is ready for auto-scheduling. **Marcos Monti** was selected for due date: **2026-01-13 (7 workdays out)** based on earliest availability. At the time of execution, they had **360 total mins available** and **120 mins already scheduled**. After this task assignment, Marcos Monti will be left with **120 mins available**. The execution found **25 possible option(s)** that passed filters."
```

The enriched narrative now includes:
- **Why the designer was chosen**: Previous designer status, preference scores, account preferences
- **Availability details**: Total available time, scheduled time, remaining time after assignment
- **Selection context**: Number of options considered, decision priority used

**Implementation**:
- `process_single_task()` now includes `decision_log` from master result (router.py:800-811)
- Router narrative incorporates `decision_log` for single tasks (router.py:1530-1537)
- Batch processing includes individual decision summaries for each assigned task (router.py:1093-1106, 1578-1591, 1611-1624)
...
<details>
<summary><h2>🔧 Script Reference</h2></summary>
...
<details>
<summary><h2>🧪 Dev Mode (Added 2025-12-18)</h2></summary>
...
<details>
<summary><h2>⏱️ Time Estimate Flow (Added 2025-12-18)</h2></summary>
...
<details>
<summary><h2>🗄️ Database Tables</h2></summary>
...
<details>
<summary><h2>🔧 Core Functions Reference (Added 2025-12-18)</h2></summary>
...
<details>
<summary><h2>🔌 Supabase RPC Functions</h2></summary>
...
### Fixed: RPC Function Overloading Error (2025-12-19)

**Issue**: `PGRST203` error when calling `as_get_assignees_data` RPC:
```
Could not choose the best candidate function between:
- public.as_get_assignees_data(input_task_id => text)
- public.as_get_assignees_data(input_task_id => text, input_account => text, ...)
```

**Root Cause**: PostgreSQL/PostgREST cannot disambiguate between two function signatures when one is a subset of another with default parameters.

**Solution**: Dropped the simple version of `as_get_assignees_data(input_task_id text)` and kept only the complex version with all optional parameters. The complex version handles both use cases:
- Task ID mode: `as_get_assignees_data(input_task_id => 'task123')`
- Account/Tag mode: `as_get_assignees_data(input_account => '306', input_tag => 'sermonseries')`

**Files Changed**:
- **Supabase**: Dropped `as_get_assignees_data(input_task_id text)` via migration
- **Backup**: Saved to `supabase_queries/rpc/as_get_assignees_data_simple_version_backup_2024_12_19.sql`

**Migration**:
```sql
-- Drop the simple version to resolve overloading conflict
DROP FUNCTION IF EXISTS public.as_get_assignees_data(input_task_id text);
```

---

### Fixed: Python None Handling in task.py (2025-12-19)

**Issue**: `AttributeError: 'NoneType' object has no attribute 'lower'` when processing assignee data.

**Root Cause**: `.get("key", "default")` returns None if the key exists but has a None value, bypassing the default.

**Solution**: Changed from `.get("username", "").lower()` to `(assignee.get("username") or "").lower()` to properly handle None values.

**Files Changed**:
- `f/aa26/task.py` lines 196-197
...
**After**:
```python
username = (assignee.get("username") or "").lower()
