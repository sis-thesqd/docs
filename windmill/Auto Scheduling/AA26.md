...
### 6. Queue Script (Added 2025-12-18)
...
1. Fetch all data in parallel:
   - Queue task data (account info, non-employee assignees)
   - Task processing data (task_name, estimate, department, tag, etc.)
   - ClickUp task (to check if subtask)
2. Calculate next queue number for the account
3. Upsert to as_log with all available fields
4. Add "queued" tag to ClickUp task
5. Send queue notification comment to each non-employee assignee

**Key Functions** (core.py):
- `fetch_queue_task_data()`: Gets account info (high_usage, cap, room, mhs3, etc.) and filters out employee assignees
- `fetch_task_data()`: Gets task metadata (task_name, estimate, department, tag, min/max_days_out)
- `fetch_clickup_task()`: Gets ClickUp task to determine if subtask
...
### Move Task to Queue (Added 2025-12-18)
```bash
...
### Scenario 6: Move Task to Queue (Added 2025-12-18)

**Goal**: Add a task to the queue when the account has no room

**Steps**:
1. Call the queue script with the task_id:
```bash
wmill script run f/aa26_v2/queue --data '{"task_id": "86dyu6p54"}'
```

**What Happens**:
1. Script fetches account info from `as_get_task_assignment_data` RPC
2. Calculates `queue_num` (count of queued tasks for that account + 1)
3. Upserts to `as_log` with `status = "queued"` and `queue_num`
4. Adds "queued" tag to the ClickUp task
5. Sends queue notification comment to each non-employee assignee
