...
**Key Classifications**:
- `auto_schedule`: Task ready for auto-scheduling
- `queued`: Task queued due to no room (process=False)
...
- `move_subtasks`: Parent assigned, subtasks unassigned
- `move_dependent_tasks`: Deliverables needed, dependent tasks unassigned

**Completed Parent Task Logic** (Added 2025-12-16):

When the input task's status is "closed" or "final files delivered" (case-insensitive):
...
```json
{
  ...
  "process": true,
  "reason": "Parent task 'closed' completed, 3 queued task(s) ready for processing",
  "account_details": {
    "account": "1234",
    "cap": 10,
    "active_tasks": 5,
