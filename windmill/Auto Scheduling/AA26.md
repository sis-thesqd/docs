...
**Processing Steps**:
1. Fetch task data including account and non-employee assignees
2. Calculate next queue number for the account
3. Upsert to as_log with `status = "queued"` and `queue_num`
4. Add "queued" tag to ClickUp task
5. Send queue notification comment to each non-employee assignee

**Key Functions** (core.py):
- `fetch_queue_task_data()`: Gets account info and filters out employee assignees
- `get_queue_num_for_account()`: Counts current queued tasks + 1
- `upsert_queue_to_as_log()`: Updates as_log with queued status
...
**Database Upsert**: Writes to `as_log` table with `status = "queued"` and `queue_num` (core.py)

**Dependencies**:
- Calls `f/clickup/send-comment` via Windmill client for comment notifications
- Uses ClickUp API to add "queued" tag
